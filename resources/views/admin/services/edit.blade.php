...
<div class="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4 animate-fade-in" style="animation-delay: 0.05s;">
    <div>
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Service</h1>
        <p class="text-sm text-slate-500 mt-1">Update details and pricing for this service.</p>
    </div>
...
<form action="{{ route('admin.services.update', $service) }}" method="POST" class="flex flex-col lg:flex-row gap-8 items-start animate-fade-in" style="animation-delay: 0.1s;">
    @csrf
    @method('PUT')
    ...
    <div class="flex-1 w-full bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="p-6 space-y-5">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $service->name) }}" class="block w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#0f2557]/20 focus:border-[#0f2557] sm:text-sm transition-all outline-none bg-white" required>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="number" name="duration_minutes" value="{{ old('duration_minutes', $service->duration_minutes) }}" min="1" class="block w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#0f2557]/20 focus:border-[#0f2557] sm:text-sm transition-all outline-none bg-white" required>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="price_rwf" value="{{ old('price_rwf', $service->price_rwf ? round($service->price_rwf) : '') }}" min="0" placeholder="5000" class="block w-full pl-12 pr-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#0f2557]/20 focus:border-[#0f2557] sm:text-sm transition-all outline-none bg-white">
                    </div>
            ...
            <div>
                ...
                <textarea name="description" rows="4" class="block w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#0f2557]/20 focus:border-[#0f2557] sm:text-sm transition-all outline-none bg-white" required>{{ old('description', $service->description) }}</textarea>
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-between">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" class="sr-only peer" {{ old('is_active', $service->is_active) ? 'checked' : '' }}>
            ...
            <div class="p-6">
                ...
                <div class="max-h-[500px] overflow-y-auto space-y-4 pr-2 custom-scrollbar">
                    @forelse($cities as $city)
                        @php
                            $pivot = $service->cities->where('id', $city->id)->first();
                            $existingPrice = $pivot ? $pivot->pivot->price_rwf : null;
                        @endphp
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="city_prices[{{ $city->id }}]" value="{{ old('city_prices.'.$city->id, $existingPrice) }}" min="0" placeholder="Base Price" class="block w-full pl-12 pr-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#0f2557]/20 focus:border-[#0f2557] sm:text-sm transition-all outline-none bg-white">
                        </div>
                    </div>
                    @empty
