...
<div class="bg-[#fafafc] min-h-screen pb-24 -mt-8 relative z-20">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="space-y-4" x-data="{ activeAccordion: null }">
            @php
                // Group 'spa', 'facial', and 'waxing' together for the frontend
                $spaGroupSlugs = ['spa', 'facial', 'waxing'];
                $spaCategories = $categories->filter(fn($c) => in_array($c->slug, $spaGroupSlugs));
                $otherCategories = $categories->filter(fn($c) => !in_array($c->slug, $spaGroupSlugs));
                
                $displayGroups = collect();
                if ($spaCategories->count() > 0) {
                    $spaMain = $spaCategories->firstWhere('slug', 'spa') ?? $spaCategories->first();
                    $displayGroups->push([
                        'id' => 'spa_group',
                        'name' => 'Spa & Wellness',
                        'image_path' => $spaMain->image_path,
                        'description' => 'Comprehensive spa, facial, and waxing services.',
                        'total_services' => $spaCategories->sum(fn($c) => $c->services->count()),
                        'is_group' => true,
                        'subcategories' => $spaCategories
                    ]);
                }
                foreach ($otherCategories as $cat) {
                    $displayGroups->push([
                        ...
                        'total_services' => $cat->services->count(),
                        'is_group' => false,
                        'subcategories' => collect([$cat])
                    ]);
                }
            @endphp
            ...
            @forelse($displayGroups as $group)
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden transition-all duration-300"
                     :class="activeAccordion === '{{ $group['id'] }}' ? 'ring-2 ring-[#0f2557]/20 shadow-md' : 'hover:border-slate-300'">
                    
                    <!-- Accordion Header -->
                    <button @click="activeAccordion = activeAccordion === '{{ $group['id'] }}' ? null : '{{ $group['id'] }}'" 
                        ...
                        <div class="flex items-center gap-4 text-left">
                            @if($group['image_path'])
                                <img src="{{ asset($group['image_path']) }}" alt="{{ $group['name'] }}" class="w-12 h-12 rounded-full object-cover border border-slate-100 shadow-sm hidden sm:block">
                            @endif
                            <div>
                                <h2 class="font-display text-xl font-bold text-slate-900">{{ $group['name'] }}</h2>
                                <p class="text-sm text-slate-500 font-medium mt-0.5">{{ $group['total_services'] }} Treatments</p>
                            </div>
                        </div>
                        <div class="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 transition-transform duration-300 border border-slate-100"
                             :class="activeAccordion === '{{ $group['id'] }}' ? 'rotate-180 bg-blue-50 text-[#0f2557] border-blue-100' : ''">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                        </div>
                    </button>

                    <!-- Accordion Content -->
                    <div x-show="activeAccordion === '{{ $group['id'] }}'" x-transition x-cloak>
                        <div class="border-t border-slate-100">
                            ...
                            <div class="bg-white" x-data="{ innerAccordion: '{{ $group['is_group'] ? $group['subcategories']->first()->id : '' }}' }">
                                @foreach($group['subcategories'] as $category)
                                    
                                    @if($group['is_group'])
                                        <button @click="innerAccordion = innerAccordion === '{{ $category->id }}' ? null : '{{ $category->id }}'" 
                                                class="w-full px-6 py-3 flex items-center justify-between bg-slate-50 hover:bg-slate-100 border-b border-slate-200 transition-colors focus:outline-none">
                                            <div class="font-bold text-[#0f2557]">{{ $category->name }} ({{ $category->services->count() }})</div>
                                            <svg class="w-4 h-4 text-slate-400 transition-transform duration-300"
                                                 :class="innerAccordion === '{{ $category->id }}' ? 'rotate-180' : ''" 
                                    ...
                                    <div x-show="!{{ $group['is_group'] ? 'true' : 'false' }} || innerAccordion === '{{ $category->id }}'" x-transition x-cloak>
